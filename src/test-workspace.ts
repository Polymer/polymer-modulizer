...
export async function testWorkspace(
  ...
  const totalResults = await run([...setupRepoResults.successes.keys()], async (repo) => {
    logStep(2, 5, '🔧', `Installing Dependencies...`);
    const installResults = await installNpmDependencies([repo]);
    ...
    const restoreResults = await restoreRepos([repo]);
  ...
  return [...totalResults.successes.keys()];
}
