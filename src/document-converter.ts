...
export class DocumentConverter {
  ...
  constructor(
      document: Document<ParsedHtmlDocument>, urlHandler: UrlHandler,
      conversionSettings: ConversionSettings, selfScanResult?: ScanResult) {
  ...
  scanJsModule(): DeleteFileScanResult|JsModuleScanResult {
    ...
    return {
      ...
      convertedFilePath: replaceHtmlExtensionIfFound(this.convertedUrl) as
   ...
   */
  convertJsModule(scanResults: ScanResults, namespacedExports: Map<string, JsExport>):
      ConversionResult[] {
  ...
  scanTopLevelHtmlDocument(): HtmlDocumentScanResult {
    return {
      ...
      convertedUrl: this.convertedUrl,
      originalUrl: this.originalUrl,
      convertedFilePath: this.originalUrl as string as ConvertedDocumentFilePath,
    };
  }
   ...
   */
  convertTopLevelHtmlDocument(scanResults: ScanResults, namespacedExports: Map<string, JsExport>): ConversionResult {
    ...
    for (const script of this.document.getFeatures({kind: 'js-document'})) {
      ...
      const program = this.rewriteInlineScript(scanResults, file.program, namespacedExports);
    ...
    for (const scriptImport of this.document.getFeatures(
      ...
      const convertedUrl = this.convertDocumentUrl(
          this.urlHandler.getDocumentUrl(scriptImport.document), scanResults);
      const formattedUrl =
   ...
   */
  private rewriteInlineScript(scanResults: ScanResults,
  ...
  private addJsImports(
    ...
    for (const htmlImport of this.getHtmlImports()) {
      const importedJsDocumentUrl = this.convertDocumentUrl(
          this.urlHandler.getDocumentUrl(htmlImport.document), scanResults);
